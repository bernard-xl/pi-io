...
void serl_updated(io_channel_t *channel, io_handle_t *subscriber, int status) {
   if(status == -1) {
      if(tcp1.fd == -1) {
          puts("tcp1 not connected.");
      } else perror("serl_updated");
   } else {
      puts("arduino sent to tcp1.");
   }
}

void backup_updated(io_channel_t *channel, io_handle_t *subscriber, int status) {
   if(status == -1) {
      if(backup.fd == -1) {
          puts("log file not opened.");
      } else perror("backup_updated");
   } else {
      puts("backup recorded.");
   }
}

void backup_sent(io_handle_t *from, io_handle_t *to, int status) {
   if(status == -1) perror("backup_sent");
   else printf("%d bytes of backup log sent.\n", status);
}
...
void tcp3_alloc(io_handle_t *handle, io_buf_t *buf) {
      buf->base = storage_alloc(&storage);
      buf->len = (buf->base)? 512 : 0;
}
...
void data_published(io_handle_t *handle, int status) {
   if(status == 0) {

...
void tcp1_spliced(io_handle_t *from, io_handle_t *to, int status) {
   if(status == -1) {
      if(serl.fd == -1) puts("arduino not connected.");
...
void tcp2_spliced(io_handle_t *from, io_handle_t *to, int status) {
   if(status == -1) {
      if(rfcm.fd == -1) puts("bluetooth not connected.");
...
void serl_spliced(io_handle_t *from, io_handle_t *to, int status) {
   if(status == -1) {
      if(tcp1.fd == -1) puts("tcp1 not connected.");
...
void file_changed(io_handle_t *handle, const char *path, int events) {
   ...
   if((events & IN_CREATE) && serl.fd == -1) {
      ...
      if(io_serial(&loop, &serl, addr, B115200, SERIAL_8N1) == -1)  
...
int main(int argc, char **argv) {
   ...
   io_handle_t serv1, serv2, servr, watcher;
   ...
   if(try_serial(&loop, &serl) == -1)
      puts("warning: arduino not connected.");
   else
      DIE_IF_ERR(io_splice_start(&serl, &tcp1, serl_spliced));
